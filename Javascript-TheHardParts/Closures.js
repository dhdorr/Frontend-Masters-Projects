...
function dateStamp(func) {
	let myObj = {"date": "", "output": 0};
  
  function myFunc(arg) {
    // do something
    return {"date": new Date(arg).toString(), "output": func(arg)};
  }
...
function censor() {
  ...
  function myFunc(arg1, arg2) {
    ...
    if (arg2) {
      ...
      let finalString = arg1;
      for (let i in myObj) {
        finalString = finalString.replace(i, myObj[i]);
      }
      return finalString;
    }
  }
...
function makeFuncTester(arrOfTests) {
  ...
  function myFunc(func) {
    ...
    return testBool;
		...
		// for(let w = 0; w < arrOfTests.length; w++) {
		// if(func(arrOfTests[w][0]) !== arrOfTests[w][1]) {
		// testBool = false;
		// return false;
		// }
		// }
...
function makeHistory(limit) {
  ...
  function myFunc(arg) {
  	// do something
    if (arg === 'undo') {
      if (myActions.length > 0 && count > 0) {
        count--;
        let temp = myActions.pop();
